...
export default function ConnectorQualityPage() {
  ...
  const kpi = useMemo(() => {
    const sums = (procs: Set<string>) =>
      filtered
        .filter((r) => procs.has(r.process))
        .reduce(
          (acc, r) => ({
            total: acc.total + r.qty_total,
            defect: acc.defect + r.qty_defect,
          }),
          { total: 0, defect: 0 },
        );

    const proc = sums(NON_INSPECTION_PROCESSES);
    const insp = sums(INSPECTION_PROCESSES);
    const processPpm = ppm(proc.defect, proc.total);
    const inspectionPpm = ppm(insp.defect, insp.total);
    const achievementPct =
      inspectionPpm === null || inspectionPpm === 0
        ? null
        : Math.round((QUALITY_TARGET_PPM / inspectionPpm) * 100);
