...
// 품질목표는 참고 화면에서 3,190 PPM 으로 고정돼 있다. 원본 화면은 이 값이
// 달에 따라 계단식으로 바뀌는 것처럼 보였지만(사람이 상/하반기 목표를 따로
// 잡아둔 흔적), 커넥터/전자빔과 같은 이유로 현재 값 하나로 단순화했다.
const QUALITY_TARGET_PPM = 3190;

const INSPECTION_PROCESSES = new Set(["V/M 검사"]);
const NON_INSPECTION_PROCESSES = new Set(["권선", "절단", "코일링", "프레스", "토션기"]);
...
export default function VmQualityPage() {
  const [records, setRecords] = useState<VmRecord[] | null>(null);
  ...
  useEffect(() => {
    Promise.all([
      fetchAll<VmRecord>(
        "vm_quality_records",
        "work_date, process, part_number, shape, qty_total, qty_defect",
      ),
      fetchAll<DefectDetail>(
        "vm_quality_defect_details",
  ...
  const dailyTrend = useMemo(() => {
    const byDate = new Map<string, VmRecord[]>();
  ...
  const shapeDefectRate = useMemo(() => {
    return shapes
      .map((shape) => {
        const recs = filtered.filter((r) => r.shape === shape);
        const total = recs.reduce((a, r) => a + r.qty_total, 0);
        const defect = recs.reduce((a, r) => a + r.qty_defect, 0);
        return { shape, ppm: ppm(defect, total) ?? 0 };
      })
      .sort((a, b) => b.ppm - a.ppm);
  }, [shapes, filtered]);

  const shapeShare = useMemo(() => {
    ...
    return shapes
      .map((shape) => {
        ...
        return { name: shape, value: qty, pct: Math.round((qty / totalQty) * 1000) / 10 };
      })
      .filter((d) => d.value > 0)
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <h1 className="text-lg font-semibold">VM코일 품질실적</h1>
        <LastSyncBadge table="vm_quality_records" />
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-lg border border-black/10 p-4 dark:border-white/10">
          <p className="mb-4 text-sm font-semibold">형상별 생산 비율</p>
          {shapeShare.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                ...
                >
                  {shapeShare.map((d) => (
                    <Cell key={d.name} fill={SHAPE_COLORS[d.name] ?? "#0891b2"} />
                  ))}
                </Pie>
                <Tooltip {...TOOLTIP_PROPS} formatter={labelNumber} />
              </PieChart>
      ...
      <div className="rounded-lg border border-black/10 dark:border-white/10">
        <div className="border-b border-black/10 px-4 py-3 dark:border-white/10">
          <p className="text-sm font-semibold">VM 검사공정 세부 불량유형</p>
        </div>
